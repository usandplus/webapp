...
interface BusinessRequest {
  ...
  email: string;
  rfc: string;
  type: UNPBaseEntityType; // e.g., 'empresa'
  createdBy: string;
  ownerId: string;
  ownerDisplayName: string;
}

const NewBusinessView: React.FC = () => {
  ...
  const validateForm = (data: BusinessRequest): boolean => {
    const newErrors = {
  ...
  const handleSubmit = async () => {
    ...
    if (result.success) {
      ...
      setSuccess(true)
      setModalMessage('¡Tu empresa ha sido registrada con éxito!')
      setLoading(false)
    } else {
      setSuccess(false)
    ...
    <Container className="py-4">
      ...
      <Modal show={showModal} onHide={closeModal} centered>
        <Modal.Header closeButton>
          <Modal.Title>Registro de Business</Modal.Title>
        </Modal.Header>
