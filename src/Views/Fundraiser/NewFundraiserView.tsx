...
interface FundraiserRequest {
  ...
  email: string;
  rfc: string;
  type: UNPBaseEntityType; // e.g., 'convocatoria'
  createdBy: string;
  ownerId: string;
  ownerDisplayName: string;
}

const NewFundraiserView: React.FC = () => {
  const { user } = useAuthContext();
  const navigate = useNavigate()
  const [formData, setFormData] = useState<FundraiserRequest>({
  ...
  const validateForm = (data: FundraiserRequest): boolean => {
    const newErrors = {
    ...
    <Container className="py-4">
            ...
            <Card.Body>
              ...
              <Form>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    type="email"
                    placeholder="correo@tuconvocatoria.com"
                    name="email"
      ...
      <Modal show={showModal} onHide={closeModal} centered>
        <Modal.Header closeButton>
          <Modal.Title>Registro de Fundraiser</Modal.Title>
        </Modal.Header>
...
export default NewFundraiserView;
