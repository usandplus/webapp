...
interface FundacionRequest {
  name: string;
  displayName: string;
  email: string;
  rfc: string;
  type: UNPBaseEntityType; // e.g., 'fundacion'
  createdBy: string;
  ownerId: string;
  ownerDisplayName: string;
}

const NewFundacionView: React.FC = () => {
  const { user } = useAuthContext();
  const navigate = useNavigate()
  const [formData, setFormData] = useState<FundacionRequest>({
  ...
  const validateForm = (data: FundacionRequest): boolean => {
    const newErrors = {
  ...
  const handleSubmit = async () => {
    ...
    if (result.success) {
      ...
      setSuccess(true)
      setModalMessage('¡Tu fundación ha sido registrada con éxito!')
      setLoading(false)
    } else {
      setSuccess(false)
    ...
    <Container className="py-4">
            ...
            <Card.Body>
              ...
              <Form>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    type="email"
                    placeholder="correo@tufundacion.com"
                    name="email"
      ...
      <Modal show={showModal} onHide={closeModal} centered>
        <Modal.Header closeButton>
          <Modal.Title>Registro de Fundacion</Modal.Title>
        </Modal.Header>
...
export default NewFundacionView;
